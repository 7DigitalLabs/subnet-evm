...
func TestEqual(t *testing.T) {
	...
	tests := map[string]testutils.ConfigEqualTest{
		...
		"different type": {
			Config:   NewConfig(nil, nil, nil, nil, nil),
			Other:    precompileconfig.NewMockConfig(gomock.NewController(t)),
			Expected: false,
		},
		"different timestamp": {
			Config:   NewConfig(utils.NewUint64(3), admins, enableds, managers, nil),
			Other:    NewConfig(utils.NewUint64(4), admins, enableds, managers, nil),
			Expected: false,
		},
		"same config": {
			Config:   NewConfig(utils.NewUint64(3), admins, enableds, managers, nil),
			Other:    NewConfig(utils.NewUint64(3), admins, enableds, managers, nil),
			Expected: true,
		},
	}
