...
func (pool *TxPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(pool.currentState.GetBalance(addr), pool.currentMaxGas.Load(), pool.currentState)
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(pool.currentState.GetBalance(addr), pool.currentMaxGas.Load(), pool.currentState)
		for _, tx := range drops {
			hash := tx.Hash()
